...
- UUIDs (v7) everywhere to avoid ID enumeration and preserve insertion order.
- All frontend text content lives in `src/data/` (no hardcoded strings in components).

### Tenancy / authorization model
**v1 is multi-tenant by ownership.** Every photo, album, and tag is owned by the user that created it via API. Authorization rules:
- Public **read** of any photo/album/tag (no auth) — the gallery is browsable by anyone.
- **Mutating** routes require Sanctum auth AND ownership: a user may only PATCH/DELETE rows where `user_id = auth()->id()`. Filament admin users bypass ownership (admin role).
- Filament admin operates under the `web` guard; authenticated panel users are implicitly admins.

This is enforced via Laravel policies (`PhotoPolicy`, `AlbumPolicy`, `TagPolicy`) registered in `AuthServiceProvider`. Every controller method calls `$this->authorize($action, $model)` before mutating.
...
│   ├── Contracts/
│   │   ├── ImageProcessor.php              # interface — see §11.2a
│   │   └── ExifExtractor.php
│   ├── Concerns/
│   │   └── HasUuidV7.php                   # UUID v7 trait (overrides HasUuids)
│   ├── Events/
│   │   ├── PhotoUploaded.php
│   │   ├── PhotoProcessed.php
│   │   ├── PhotoDeleted.php
...
│   ├── Listeners/                          # domain event handlers (stats, search reindex, future webhooks)
│   ├── Models/
│   │   ├── Album.php
│   │   ├── Photo.php
│   │   ├── Tag.php
...
│   │   └── PhotoPolicy.php
│   ├── Providers/
│   │   ├── AppServiceProvider.php          # binds ImageProcessor + ExifExtractor contracts
│   │   ├── AuthServiceProvider.php         # registers policies
│   │   └── Filament/AdminPanelProvider.php
│   ├── Queries/
│   │   └── PhotoQuery.php                  # composable filters/sorts shared by API + Filament
│   └── Services/
│       ├── Imaging/
...
│       └── TagAssigner.php                  # syncByNames(Photo, array) — used by API + Filament
...
│   ├── migrations/                         # timestamps generated by `php artisan make:migration`
│   │   ├── <ts>_create_users_table.php                         # adds is_admin BOOLEAN default false
│   │   ├── <ts>_create_albums_table.php                        # user_id FK → users; no cover_photo_id FK yet
│   │   ├── <ts>_create_tags_table.php
│   │   ├── <ts>_create_photos_table.php                        # user_id FK; album_id FK; FULLTEXT(title, description)
│   │   ├── <ts>_add_cover_photo_fk_to_albums.php               # resolves circular FK
│   │   ├── <ts>_create_photo_tag_table.php
│   │   ├── <ts>_create_personal_access_tokens_table.php        # tokenable_id CHAR(36), not BIGINT
...
│   │   ├── common/
│   │   │   ├── Button.tsx
│   │   │   ├── Modal.tsx                    # uses react-focus-lock; aria-modal
│   │   │   ├── EmptyState.tsx
│   │   │   ├── Spinner.tsx
│   │   │   ├── ConfirmDialog.tsx
│   │   │   └── ErrorBoundary.tsx            # wraps pages + interactive overlays
│   │   ├── layout/
│   │   │   ├── Navbar.tsx
│   │   │   ├── Sidebar.tsx
...
│   │   └── providers/
│   │       └── KeyboardShortcutsProvider.tsx   # singleton — see §8.0 rule 13
│   ├── data/
│   │   ├── copy.ts                         # all UI strings
│   │   ├── nav.ts
│   │   ├── shortcuts.ts                    # keyboard shortcut definitions (key → action name)
│   │   └── polling.ts                      # centralized intervals (BATCH_MS, PHOTO_MS, QUEUE_MS)
│   ├── hooks/
│   │   ├── useAuth.ts
│   │   ├── useDebounce.ts
│   │   ├── useKeyboardShortcuts.ts          # consumer hook for the provider above
│   │   ├── usePhotos.ts                    # TanStack Query wrappers
│   │   ├── useAlbums.ts
│   │   ├── useTags.ts
│   │   ├── useUpload.ts
│   │   └── useBatchPoll.ts                  # single polling loop after upload (see §8.7)
│   ├── pages/
│   │   ├── GalleryPage.tsx
│   │   ├── AlbumPage.tsx
│   │   ├── FavoritesPage.tsx
...
│   ├── App.tsx
│   ├── main.tsx
│   └── index.css                           # @import "tailwindcss"; @theme { ... }; prefers-reduced-motion rules
├── tests/
...
- All primary keys are **UUID v7** strings, stored as `CHAR(36)`, **NOT NULL**, no default. Generated at model boot via an override:
  ```php
  // App\Concerns\HasUuidV7
  public function newUniqueId(): string { return (string) Str::uuid7(); }
  ```
  UUID v7 is sortable by creation time, so `ORDER BY id` ≈ `ORDER BY created_at` — useful for cursor pagination. Default Laravel `HasUuids` trait emits v4; this project overrides it.
...
- The `exif` JSON column is opaque storage — **never** `WHERE exif->...` query it. EXIF is for display only.
- **Soft deletes are out of scope for v1.** Deletes are permanent. If undo is required later, add `deleted_at` columns + `SoftDeletes` trait via a future migration.
...
| name | VARCHAR(255) | NOT NULL | — | — (unique per user — see composite UNIQUE below) |
...
**Indexes:**
- PRIMARY (`id`)
- UNIQUE (`user_id`, `name`) `albums_user_name_unique` — name is unique per owner, not globally
...
**Rationale for per-user UNIQUE name:** two different users can both have an album called "Travel" without colliding.
...
**Tags are global** (no `user_id`). The tag vocabulary is shared across all users — two users tagging photos as "sunset" reference the same `tags.id`. This is intentional: tags act as a folksonomy. Tag *creation* requires Sanctum auth (any authenticated user can introduce a new tag via photo upload), but tags are never owned and cannot be deleted via the API. Filament admins manage the global tag list.
...
|---|---|---|---|---|
| id | CHAR(36) | NOT NULL | — | PRIMARY KEY (UUID v7) |
| user_id | CHAR(36) | NOT NULL | — | FK → `users(id)` · ON DELETE **CASCADE** · ON UPDATE **CASCADE** (owner — see §1 tenancy) |
| title | VARCHAR(255) | NOT NULL | — | — |
| description | TEXT | NULL | NULL | — |
| filename | VARCHAR(255) | NOT NULL | — | — (original user-supplied filename) |
| original_path | VARCHAR(500) | NOT NULL | — | — (private disk path; never publicly served — see §11.1) |
| thumbnail_path | VARCHAR(500) | NULL | NULL | — (populated by `ProcessPhoto` job; public) |
| medium_path | VARCHAR(500) | NULL | NULL | — (populated by `ProcessPhoto` job; public) |
| large_path | VARCHAR(500) | NULL | NULL | — (populated by `ProcessPhoto` job; public) |
...
| exif | JSON | NULL | NULL | — (sanitized — GPS coords stripped before persistence; see §11.2) |
...
- INDEX `photos_album_id_created_at_idx` (`album_id`, `created_at`) — covers "newest in album" queries
- INDEX `photos_is_favorite_created_at_idx` (`is_favorite`, `created_at`) — covers `favorites_first` sort and favorites listing
- INDEX `photos_processing_status_created_at_idx` (`processing_status`, `created_at`) — covers queue dashboard
- INDEX `photos_created_at_idx` (`created_at`) — default "newest" sort across all photos
- FULLTEXT INDEX `photos_search_idx` (`title`, `description`) — used by `search` query param via `MATCH ... AGAINST` (MySQL/MariaDB; for PostgreSQL use `tsvector` GIN index)

**Rationale for cascade behaviour:**
- `user_id` ON DELETE **CASCADE** — deleting a user removes all their photos (and the file cleanup runs via `PhotoObserver`).
- `album_id` ON DELETE **SET NULL** — deleting an album preserves the photos; they become unassigned.

**EXIF sanitization:** before persisting `exif` JSON, GPS coordinates (`GPSLatitude`, `GPSLongitude`, `GPSAltitude`) are stripped. Other fields (camera, ISO, aperture, shutter, focal_length, taken_at) are preserved. Originals on disk also have GPS stripped before being written (see §11.2).
...
// User
public function photos(): HasMany            // → Photo (owned)
public function albums(): HasMany            // → Album (owned)
...
- Deleting a `Tag` → `photo_tag` rows removed (CASCADE); photos survive. Tag deletion is admin-only (no API endpoint).
- Deleting a `User` → all owned photos and albums cascade-delete (CASCADE), which transitively triggers the photo/album cascades above; personal access tokens cascade via Sanctum's morph.

**Authorization:** `PhotoPolicy::update($user, $photo)` and `::delete($user, $photo)` return `$user->id === $photo->user_id || $user->isAdmin()`. Same shape for `AlbumPolicy`. Tags have no policy (admin-only writes).
...
- **Rate limits:** every endpoint is throttled by **both** IP and authenticated user:
  - `/auth/*` — `throttle:10,1` (per IP) — 10 req/min
  - everything else — `throttle:120,1` (per IP) **AND** `throttle:300,1,user` (per authenticated user)
  - Exceeding either limit → `429 rate_limited`.
...
- **Response envelope is uniform.** Every endpoint wraps its payload in `{ "data": ... }`. No exceptions — including auth, favorite mark/unmark, and batch endpoints. This was inconsistent in earlier drafts and has been normalized.
- **Pagination envelope** (applied to every list endpoint, layered on top of the uniform envelope):
  ```json
  {
    "data": [ /* Resource[] */ ],
    "links": {
      "first": "https://api.example.com/api/v1/photos?cursor=...",
      "next":  "https://api.example.com/api/v1/photos?cursor=...",
      "prev":  null
    },
    "meta": {
      "per_page": 24,
      "next_cursor": "eyJpZCI6IjAxOTM...",
      "prev_cursor": null,
      "path": "https://api.example.com/api/v1/photos"
    }
  }
  ```
  Pagination is **cursor-based** (`cursorPaginate()`), not offset-based — UUID v7 ids sort by creation time, so cursors are stable even as new rows arrive. There is no `total` count (cursors don't need it).
- **Single-resource envelope:** `{ "data": { /* Resource */ } }`.
- **Timestamps:** ISO-8601 UTC (`"2026-04-24T12:00:00Z"`).
- **IDs:** UUID v7 strings.
- **Caching:** every `GET /photos/{id}` and `GET /albums/{id}` emits `ETag: W/"<sha1(updated_at)>"`. Clients sending `If-None-Match` with the same value get `304 Not Modified` (empty body).
...
- **Success `200 OK`** (wrapped): same shape as `/auth/register`.
- **Errors:**
...
#### `GET /photos`
Paginated list with filters, search, and sorting. Cursor-based.
...
- **Query parameters:**
  ...
  |---|---|---|---|---|
  | `search` | string | no | `null` | 1–100 chars. Server runs `MATCH(title, description) AGAINST (?)` against the `photos_search_idx` FULLTEXT index. |
  ...
  | `sort` | enum | no | `created_at` | One of: `created_at`, `title`, `favorites`. |
  | `order` | enum | no | `desc` | `asc` or `desc`. |
  | `per_page` | integer | no | `24` | Min `1`, max `100`. |
  | `cursor` | opaque string | no | `null` | Returned by previous response in `meta.next_cursor` / `meta.prev_cursor`. |

- **Success `200 OK`** — pagination envelope where `data` is an array of the **Photo resource** (§6.5). Relations always eager-loaded: `with(['album:id,name', 'tags:id,name,slug'])`.
- **Errors:**
  - `422` — invalid query param (unknown `sort`/`order`, bad UUID, `per_page` out of range, unknown tag slug).
...
#### `POST /photos`
Upload one or more photos and queue processing. **Single endpoint for both single and batch uploads** (1–20 files). The previous `POST /photos/batch` is folded into this endpoint to remove client branching.

- **Auth:** token (`photos:write` ability) + ownership not applicable (creating)
- **Content-Type:** `multipart/form-data` **(required — JSON body is rejected with `415`)**
- **Form fields:**

  | Field | Type | Required | Rules |
  |---|---|---|---|
  | `files[]` | File[] | yes | 1–20 items. Each: MIME must be `image/jpeg` \| `image/png` \| `image/webp`. Size ≤ 10 MB (10240 kB). Laravel rules: `files\|array\|min:1\|max:20`, `files.*\|image\|mimes:jpg,jpeg,png,webp\|max:10240`. |
  | `titles[]` | string[] | no | Per-file titles, 1–255 chars; index-aligned with `files[]`. Missing entries default to the file's basename. |
  | `description` | string | no | 0–5000 chars. Applied to every uploaded photo (no per-file descriptions in v1). |
  | `album_id` | UUID | no | Must exist in `albums.id` and be owned by the auth'd user. Applied to every uploaded photo. |
  | `tags[]` | string[] (tag **slugs**) | no | Each must already exist in `tags.slug`. Up to 20 values. Applied to every uploaded photo. |
  | `new_tags[]` | string[] (new tag **names**) | no | Each 1–100 chars. Created if missing (slug auto-generated, collision-resolved). Up to 20 values. Applied to every uploaded photo. |
  | `is_favorite` | boolean (`0` / `1`) | no | Default `0`. Multipart booleans use `0`/`1` only. |

- **Behavior** (wrapped in `DB::transaction(...)`):
  1. Upsert any `new_tags[]` (creating rows for ones that don't exist; merging into the slug list).
  2. Sanitize each file (strip GPS EXIF) and store the original on `photos_private` disk.
  3. Insert one `Photo` row per file with `user_id = auth()->id()`, `processing_status = 'pending'`.
  4. Attach the merged tag set to each new photo.
  5. Wrap one `ProcessPhoto` job per photo in a `Bus::batch(...)`. Return the batch id.
- **Success `202 Accepted`** (wrapped):
  ```json
  {
    "data": {
      "batch_id": "uuid",
      "total": 3,
      "photos": [
        { "id": "uuid1", "title": "Sunset", "processing_status": "pending", ... },
        { "id": "uuid2", "title": "Beach",  "processing_status": "pending", ... },
        { "id": "uuid3", "title": "Boat",   "processing_status": "pending", ... }
      ]
    }
  }
  ```
  Response includes a `Location: /api/v1/photos/batch/{batch_id}` header for polling.
- **Errors:**
  - `415 unsupported_media_type` — request is not multipart, or any file MIME is not in the allowed set.
  - `413 payload_too_large` — any file exceeds 10 MB.
  - `422 validation_failed` — missing `files[]`, > 20 files, invalid `album_id`, unknown tag slug, etc.
  - `403 forbidden` — `album_id` belongs to another user.
...
- **Auth:** token (`photos:write`) + ownership (`PhotoPolicy::update`)
- **Content-Type:** `application/json`
- **Path parameters:** `id` — UUID (route-model-bound to `Photo $photo`).
- **Request body** (all optional, but at least one required):
  ...
  | `album_id` | UUID\|null | Must exist and be owned by the auth'd user; `null` unassigns from album |
  | `tags[]` | string[] (tag **slugs**) | Full replacement (not additive); empty array clears all tags. Each slug must already exist. |
  | `new_tags[]` | string[] (new tag **names**) | Created if missing. Merged into `tags[]` for the assignment. |
  | `is_favorite` | boolean (`true` / `false`) | JSON booleans use `true`/`false` only (no `0`/`1` here). |

- Wrapped in `DB::transaction(...)` (tag upsert + attach + photo update).
- **Success `200 OK`**: `{ "data": Photo }` — full Photo resource with updated fields.
- **Errors:** `403` (not owner), `404` (unknown id), `422` (validation).

---

#### `DELETE /photos/{id}`
Delete photo row and all 4 files (original on `photos_private`, three sized variants on `photos`).

- **Auth:** token (`photos:write`) + ownership
- **Path parameters:** `id` — UUID (route-model-bound).
- **Success `204 No Content`** (empty body).
- **Errors:** `403`, `404`.

---

#### `PUT /photos/{id}/favorite`
Mark a photo as favorite. **Idempotent.**

- **Auth:** token (`photos:write`) + ownership
- **Request body:** empty.
- **Path parameters:** `id` — UUID.
- **Success `204 No Content`** (empty body). The new state is implicit (always `is_favorite=true` after success). Subsequent identical requests also return `204`.
- **Errors:** `403`, `404`.

#### `DELETE /photos/{id}/favorite`
Unmark a photo as favorite. **Idempotent.**
...
> **REST grammar:** the previous toggle endpoint (`POST /photos/{id}/favorite`) was non-idempotent and broke retries. It is removed in v1. Frontend uses `PUT` to add to favorites, `DELETE` to remove — both safe to retry.

---

#### `GET /photos/batch/{batchId}`
Poll batch progress. Bundles per-photo statuses so frontend only needs **one** polling loop (was two).

- **Auth:** token (must be batch creator)
- **Path parameters:** `batchId` — UUID returned by `POST /photos`.
- **Success `200 OK`** (wrapped):
  ```json
  {
    "data": {
      ...
      "finished_at": null,
      "photos": [
        { "id": "uuid1", "processing_status": "completed", "urls": { "thumbnail": "...", "medium": "...", "large": "..." } },
        { "id": "uuid2", "processing_status": "completed", "urls": { "thumbnail": "...", "medium": "...", "large": "..." } },
        { "id": "uuid3", "processing_status": "pending",   "urls": { "thumbnail": null, "medium": null, "large": null } }
      ]
    }
  }
  ```
  When `finished === true`, polling should stop. Frontend invalidates the `['photos']` query once.
- **Errors:** `403` (not the batch creator), `404` — batch ID not found (or already pruned by `queue:prune-batches`).
...
- **Success `200 OK`** (wrapped):
  ...
  ```
  Backed by `Tag::query()->withCount('photos')` to avoid N+1.
- **Errors:** `422`.

**Note:** tag creation is implicit via `POST /photos` and `PATCH /photos/{id}` using the `new_tags[]` field. There is no `POST /tags` in v1 — tags are managed through photo operations (and via the Filament admin panel).

---

### 6.4a Backend conventions (apply to every endpoint)

These rules are enforced by code review and by the `code-reviewer` agent in §13 Phase 11:

1. **Route model binding** for every `{id}` path param. Controllers receive `Photo $photo`, never `string $id`. Implicit binding handles 404; ownership check happens via `$this->authorize('update', $photo)`.
2. **`$request->validated()`** only — never `$request->all()` or `$request->input()` for fields that go into `Model::create/update`. Form Requests are the single validation surface.
3. **`DB::transaction(fn () => ...)`** wraps every multi-write operation: `POST /photos` (insert + tag attach + batch dispatch), `PATCH /photos` (update + tag sync), `POST /albums` (when cover is set), bulk Filament actions.
4. **Explicit eager loading.** Every list query declares `with([...])` and `withCount([...])` for the relations its Resource will read. N+1 is treated as a bug, caught by `Beyondcode\QueryDetector` in tests.
5. **Authorization via policies.** Every mutating controller method calls `$this->authorize($action, $model)` (or `Gate::authorize` for non-model actions). Policies live in `app/Policies/`.
6. **Sanctum abilities.** Mutating controllers call `$request->user()->tokenCan('photos:write')` (or `albums:write`); fail with `403` if missing. Defense in depth on top of policies.
7. **Domain events** for cross-cutting side effects: `PhotoUploaded`, `PhotoProcessed`, `PhotoDeleted`, `AlbumDeleted`. Listeners (in `app/Listeners/`) handle stats invalidation, future webhooks, search re-indexing. Controllers do not call listeners directly.
8. **No business logic in Filament resources.** Filament forms/tables call the same services and policies the API uses.
...
| `GET /auth/me` | `200` |
| `GET /photos` | `200`, `304` (ETag match), `422` |
...
| `GET /photos/batch/{batchId}` | `200`, `403`, `404` |
| `GET /albums` | `200`, `422` |
...
**`403 forbidden` shape:** `{ "message": "This action is unauthorized." }` — emitted by Laravel's `AuthorizationException` from the policy layer.
...
### 8.0 Frontend conventions (apply everywhere)

1. **Strict TypeScript.** `tsconfig.json` sets `"strict": true`, `"noUncheckedIndexedAccess": true`, `"noImplicitOverride": true`, `"exactOptionalPropertyTypes": true`. `any` is banned (ESLint `@typescript-eslint/no-explicit-any: error`); use `unknown` + narrowing.
2. **Error boundaries** wrap every page (`pages/*Page.tsx`) and every interactive overlay (`PhotoLightbox`, `UploadModal`). The boundary renders an inline error state with a retry button and emits a sonner toast. Implementation: a single `<ErrorBoundary>` component under `components/common/`.
3. **Code splitting** at the route level. Every page is loaded via `React.lazy(() => import('./pages/GalleryPage'))` and wrapped in `<Suspense fallback={<Spinner />}>`. Initial bundle target: < 200 KB gzip; per-route chunks: < 100 KB gzip. Enforced by a CI step using `vite-bundle-visualizer`.
4. **Server state lives in TanStack Query.** Components never duplicate server data into local state. Mutations declare their invalidations explicitly:
   ```ts
   useMutation({
     mutationFn: api.photos.update,
     onSuccess: () => queryClient.invalidateQueries({ queryKey: ['photos'] }),
   });
   ```
5. **Optimistic updates** on cheap toggles (favorite mark/unmark) via `useMutation` `onMutate` / `onError` rollback.
6. **Image rendering.** Every `<img>` declares `loading="lazy"`, `decoding="async"`, an explicit `width`/`height` (or `aspect-ratio` from photo metadata to prevent CLS), and — for masonry tiles — a `srcset` + `sizes` so the browser picks the right resolution:
   ```tsx
   <img
     src={urls.thumbnail}
     srcSet={`${urls.thumbnail} 300w, ${urls.medium} 800w`}
   ...
   />
   ```
   Above-the-fold lightbox image uses `fetchpriority="high"`.
7. **Focus management.** `Modal` and `PhotoLightbox` use `react-focus-lock` (or Radix Dialog primitives) to trap focus while open, restore focus on close, and respond to `Escape`. Every interactive element has a visible focus ring (Tailwind `focus-visible:ring-2`).
8. **A11y baseline.** Semantic HTML first; `aria-label` on icon-only buttons; alt text on every image. The `axe-core` Playwright check (added in §13 Phase 13) gates merges.
9. **Reduced motion.** `@media (prefers-reduced-motion: reduce)` in `index.css` disables non-essential transitions (lightbox slide, modal fade).
10. **No raw HTML injection.** `dangerouslySetInnerHTML` and equivalent escape hatches are forbidden by lint (`react/no-danger: error`). User-supplied descriptions are rendered as plain text only in v1.
11. **Token cache.** The auth token is read from `localStorage` once at app start and held in a module-scope variable in `src/api/client.ts`. The interceptor reads from the variable, not from `localStorage` per request. Login/logout events refresh the variable.
12. **Polling cleanup.** Every polling effect uses `useEffect` with a returned cleanup that calls `AbortController.abort()` on unmount and `clearInterval`. Never leak timers across navigation.
13. **Singleton keyboard shortcuts.** Shortcuts are registered once via a `<KeyboardShortcutsProvider>` mounted in `App.tsx`. Components consume them via `useKeyboardShortcuts({ '/': focusSearch, ... })` — registration is *declarative*, not direct event listener mounting. Prevents duplicate handlers.

### 8.1 App shell + routing
`App.tsx` wires `QueryClientProvider` + `BrowserRouter` + `<KeyboardShortcutsProvider>` + a top-level `<ErrorBoundary>`. Every route is `React.lazy`-imported and wrapped in `<Suspense>`. Routes:
...
All authenticated routes render inside `<Shell>` which renders Navbar + Sidebar + `<Outlet/>`. `<Shell>` itself is wrapped in a route-level `<ErrorBoundary>` so a page crash falls back to a recoverable UI without unmounting the chrome.
...
### 8.5 `PhotoCard`
- `<img>` follows the §8.0 rule 6 template: `loading="lazy"`, `decoding="async"`, explicit `width`/`height` from photo metadata to prevent CLS, `srcset` (`thumbnail 300w, medium 800w`), `sizes="(max-width: 768px) 50vw, 25vw"`, `alt={title}`.
- Hover overlay (desktop) / always visible on mobile: title + heart icon (favorite mark/unmark via `PUT`/`DELETE /photos/{id}/favorite`).
- **Optimistic favorite toggle.** Click flips the cached `is_favorite` immediately via `useMutation`'s `onMutate`; rolls back in `onError`.
- Click opens `PhotoLightbox` — sets `?photo=:id` in URL so lightbox is linkable.
- Processing overlay: if `processing_status === 'pending' | 'processing'`, show `ProcessingOverlay` with spinner. If `failed`, show red error icon with tooltip + "Retry" action that dispatches `POST /photos/{id}/reprocess` (admin only — out of scope for v1 user UI).

### 8.6 `PhotoLightbox`
- Portal to `<body>`, backdrop `bg-black/90`. Wrapped in `<ErrorBoundary>`.
- Focus is trapped inside the lightbox (`react-focus-lock`); restored on close. `aria-modal="true"`, `role="dialog"`, `aria-labelledby` set to the title element id.
- Displays `urls.large` with `fetchpriority="high"`. Falls back to a loading shimmer while `processing_status !== 'completed'` (the original is private and unavailable to non-owners — see §11.1).
- **Owner-only "View full size" link** — for the photo's owner, the lightbox calls `GET /photos/{id}/original-url` (see §6.2 — TBD if needed; otherwise issue signed URLs server-side and embed in the Photo resource for the owner only) to fetch a 5-minute signed URL to the private original. For non-owners, the button is hidden.
- **Prefetches** `urls.large` of the next and previous photo (TanStack Query `queryClient.prefetchQuery`) so arrow navigation is instant.
- Left/right arrows (on-screen + keyboard) cycle through current filtered list (uses same query data as grid).
- Bottom bar: title (inline editable), description (inline editable textarea), tags (chip multi-select; `tags[]` references existing slugs, `new_tags[]` for novel names), "Delete" (confirm).
- Right drawer: `ExifPanel` — key/value list from `exif` JSON (no GPS — stripped server-side per §11.2), empty state "No EXIF data available".
- Closes on backdrop click, Escape key, or `?photo=` cleared from URL. On close, focus returns to the originating `<PhotoCard>` element.
...
- `config/sanctum.php` — token expiration **24 hours** (was 30 days; shortened to limit blast radius from XSS).
- `POST /auth/login` returns `{ data: { token: string, expires_at: ISO-8601, user: User } }` (envelope per §6.0).
- Frontend stores token in `localStorage` under `pgp_token`. **Trade-off accepted with mitigations** (see §9.4).
...
### 9.2a Token abilities (Sanctum)

Tokens issued at login carry abilities matching the user's role:
| Ability | Granted to | Required for |
|---|---|---|
| `photos:write` | every authenticated user | POST/PATCH/DELETE on own photos, favorite mark/unmark |
| `albums:write` | every authenticated user | POST/PATCH/DELETE on own albums |
| `admin` | users where `users.is_admin = true` (added in a future milestone) | bypass ownership in policies; access Filament |

Controllers verify abilities with `$request->user()->tokenCan('photos:write')` in addition to ownership policies. Login request issues:
```php
$user->createToken($deviceName, ['photos:write', 'albums:write']);
```

### 9.3 CORS
`config/cors.php` — `paths: ['api/*']`, `allowed_origins` from `FRONTEND_URL` env, `supports_credentials: false` (token mode). Wildcard `*` is **forbidden** in production.

### 9.4 XSS / token storage trade-off (explicit decision)

Storing Sanctum tokens in `localStorage` is **vulnerable to XSS** — an injected script can read and exfiltrate the token. The industry-safer alternative is Sanctum *stateful cookie mode* (HttpOnly cookies), which costs us a more complex CSRF flow.

**v1 chooses `localStorage` + Bearer tokens**, mitigated by:
1. **24-hour token TTL** (not 30 days) — limits blast radius.
2. **Strict CSP** in production (set in §13 Phase 13): `default-src 'self'; img-src 'self' https://cdn.example.com data:; script-src 'self'; style-src 'self' 'unsafe-inline'; object-src 'none'; base-uri 'self'`. No third-party scripts allowed.
3. **No raw HTML injection** anywhere in the React tree — `dangerouslySetInnerHTML` (and equivalents) is banned by an ESLint rule.
4. **No user-submitted markdown rendering** in v1 (descriptions are plain text only).
5. **Token abilities** (§9.2a) — even a stolen token cannot escalate to admin.
6. **`/auth/logout` on token theft suspicion** — server-side revocation works regardless of where the token was stored.

If multi-user becomes the primary case (many anonymous attackers, low CSP control), revisit and migrate to stateful cookies.
...
**Two-disk model.** The pipeline uses two logical disks (both backed by `Storage::disk(...)`):
- `photos_private` — holds originals. Never publicly served. Local: `storage/app/photos-private/`. Production: a private S3 bucket OR private prefix `private/` of the public bucket with `Block public access`. URLs are **always** time-limited signed URLs (TTL 5 min) generated via `Storage::disk('photos_private')->temporaryUrl(...)`.
- `photos` — holds the generated thumbnail/medium/large sizes. Public-read. Local: `storage/app/public/photos/`. Production: public S3 bucket fronted by CloudFront.

This split limits original-file exposure (full EXIF, full resolution) to the photo owner and admins.

**Upload steps:**
1. `POST /photos/batch` (single endpoint after Cluster 2 — see §6.2) receives multipart file(s).
2. `StorePhotoRequest` validates: `image|mimes:jpg,jpeg,png,webp|max:10240` (10 MB in kB).
3. Controller (a) sanitizes the original by stripping GPS EXIF tags via `ExifExtractor::stripGps($file)`, (b) stores it via `Storage::disk('photos_private')->putFileAs('originals', $sanitizedFile, $uuid.'.'.$ext)`. The disk is **private** — no public URL ever generated.
4. Photo row created with `user_id = auth()->id()`, `processing_status = 'pending'`, file metadata (size, mime, original filename). Wrapped in `DB::transaction(...)` along with tag attach.
5. `ProcessPhoto::dispatch($photo)` queued.
6. Controller returns `202 Accepted` with the `Photo` resource (`Location: /api/v1/photos/{id}` header) — client does NOT wait for processing.

### 11.2 `ProcessPhoto` job
- `implements ShouldQueue`, `$tries = 3`, `$backoff = [10, 30, 60]` seconds.
- `handle()` flow:
  1. Set `processing_status = 'processing'`, save.
  2. Load original from `Storage::disk('photos_private')`.
  3. Use `ImageProcessor` service → Intervention Image v3 to generate 3 sizes:

| Size | Width | Height | Quality | Format | EXIF |
|---|---|---|---|---|---|
| thumbnail | 300 px | auto (aspect) | 80 | JPEG | **stripped** |
| medium | 800 px | auto | 85 | JPEG | **stripped** |
| large | 1600 px | auto | 90 | JPEG | **stripped** |

  Never upscale — if source < target, store source as that size (still re-encoded so EXIF is stripped). Auto-orient from EXIF before stripping.
  4. Write outputs to `Storage::disk('photos')` under `thumbnails/`, `medium/`, `large/`. These are the only public files.
  5. Extract EXIF via `ExifExtractor::extract($absolutePath)` — camera model, ISO, aperture, shutter, focal length, taken_at. **GPS coordinates are dropped** (not stored in `exif` JSON, not exposed via the API).
  6. Update `width`, `height` from source, store all paths.
  7. Set `processing_status = 'completed'`, save.
- `failed()` callback: `processing_status = 'failed'`, persist `processing_error` (exception message). Original on `photos_private` is **retained** for retry — purged only when the Photo row is deleted (via `PhotoObserver`).

### 11.2a Service contracts (DI)

```php
namespace App\Contracts;

interface ImageProcessor {
    /** Generates thumbnail/medium/large from $photo->original_path; updates $photo. */
    public function generate(\App\Models\Photo $photo): void;
}

interface ExifExtractor {
    /** Returns sanitized EXIF (no GPS) from a local file path. Returns [] on failure. */
    public function extract(string $absolutePath): array;

    /** Returns a copy of $file with all GPS EXIF tags removed. */
    public function stripGps(\Illuminate\Http\UploadedFile $file): \Illuminate\Http\UploadedFile;
}
```

Concrete implementations live under `App\Services\Imaging\`. Both are bound in `AppServiceProvider::register()`. Tests bind a fake (`tests/Doubles/FakeImageProcessor.php`) in `setUp()`.
...
`PHOTOS_DRIVER` (renamed from the misleading `PHOTOS_DISK` which was actually a driver) selects between `local` and `s3`. Two disks are configured: `photos` (public-read; sized variants only) and `photos_private` (private; originals only). Both flip drivers together based on `PHOTOS_DRIVER`.
...
# Image budget (informs queue worker --timeout)
...
'disks' => [
    ...
    'photos' => [
        ...
        'root' => storage_path('app/public/photos'),                 // local
        'url' => env('APP_URL').'/storage/photos',                   // local
        'bucket' => env('AWS_BUCKET_PUBLIC'),                        // s3
        'url' => env('AWS_URL'),                                     // s3 (CDN)
        'visibility' => 'public',
    ],
    'photos_private' => [
        'driver' => env('PHOTOS_DRIVER', 'local') === 's3' ? 's3' : 'local',
        'root' => storage_path('app/photos-private'),                // local — note: NOT under app/public
        'bucket' => env('AWS_BUCKET_PRIVATE'),                       // s3 — separate bucket with Block Public Access ON
        'visibility' => 'private',
        'throw' => true,
    ],
],
...
All file operations use `Storage::disk('photos')` or `Storage::disk('photos_private')` exclusively — no direct filesystem calls. Originals (full EXIF, full resolution) live ONLY on `photos_private`. URLs to originals are always time-limited signed URLs (5-minute TTL) generated via `Storage::disk('photos_private')->temporaryUrl(...)`, and only the photo's owner (or admin) ever receives one.

### 11.5 Workers
- **Local:** `php artisan queue:work --queue=default --tries=3 --timeout=180 --memory=512`
- **Production:** same command pointed at `sqs` connection, run under Supervisor; horizontally scalable because SQS is shared.

`--timeout=180` (was 120) accommodates `PHOTOS_MAX_DIMENSION=8000` source images on modest hardware. If you raise the dimension/size budget, raise the timeout proportionally — and update CLAUDE.md.

**Workers must run with `DB_PERSISTENT_CONNECTIONS=false`** so a connection that goes stale during a long sleep doesn't survive into the next job.

**Failed-job hygiene:** schedule `php artisan queue:flush` weekly (`app/Console/Kernel.php` → `$schedule->command('queue:flush')->weekly()`) to keep the `failed_jobs` table from growing unbounded.
...
## 14. Parallel Build Plan

§13 lists phases sequentially for clarity. In practice, with multiple developers (or Claude Code subagents in worktrees), the build compresses substantially. This section is the operator's guide to running phases in parallel.

### 14.1 Inter-phase dependency graph
...
### 14.2 Wave-by-wave execution

**Wave 0 — Sequential foundation** (no parallelism — every later phase depends on these):
1. Phase 11 (Git + PR) — set up repo + branch protection first
2. Phase 9 (Hooks) — install pre-commit hooks before any code is written
3. Phase 1 (Backend foundation: schema + models + seed)

**Wave 1 — Three parallel tracks** (kick off simultaneously after Wave 0):

| Track | Phases | Owner | Worktree path |
|---|---|---|---|
| **A — Backend API + Pipeline** | Phase 2 → Phase 6 | Agent A / Dev A | `../pgp-track-a-api` |
| **B — Filament admin** | Phase 3 | Agent B / Dev B | `../pgp-track-b-filament` |
| **C — Frontend** | Phase 4 → Phase 5 (with mocked API) | Agent C / Dev C | `../pgp-track-c-frontend` |

Tracks share zero files. Use `git worktree add ../pgp-track-X -b feat/track-X` per track.

**Sync point at end of Wave 1:** Track C swaps mocked API for the real endpoints from Track A. Run E2E smoke test against the integrated stack.

**Wave 2 — Convergence** (limited parallelism):

| Branch | Depends on | Can parallelize with |
|---|---|---|
| Phase 7 (queue monitoring) | Phase 3 ✓, Phase 6 ✓ | Phase 8, Phase 10 |
| Phase 8 (AWS migration) | Phase 6 ✓ | Phase 7, Phase 10 |
| Phase 10 (test fill-gaps) | All previous | Phase 7, Phase 8 |

**Wave 3 — Sequential close-out:**
- Phase 12 (multi-agent docs) — finalize from real experience
- Phase 13 (spec coverage audit + staging deploy)

**Wall-clock estimate:**
- Sequential build: ~15–20 working days
- Parallel build with 3 tracks: ~8–10 working days
- Bottleneck: Wave 0 (~2 days, can't be split) and Wave 3 (~1–2 days, can't be split)

### 14.3 Intra-phase parallelization

Within a single phase, further splits compress wall-clock further.

**Phase 2 (API) — 4 parallel sub-agents:**
- 2a: AuthController + AuthTest
- 2b: PhotoController CRUD + index/filtering + tests
- 2c: BatchUploadController + tests
- 2d: AlbumController + TagController + tests

**Sync point:** shared exception handler in `bootstrap/app.php` — assign to one agent first; others rebase.

**Phase 3 (Filament) — 3 parallel sub-agents:**
- 3a: PhotoResource (largest)
- 3b: AlbumResource + PhotosRelationManager + TagResource
- 3c: Widgets + StorageManagement page

**Sync point:** `AdminPanelProvider` — one agent does serial registration after parallel work lands.

**Phase 5 (React components) — 4 parallel sub-agents:**
- 5a: Layout (Shell, Navbar, Sidebar)
- 5b: Gallery (MasonryGrid, PhotoCard, ProcessingOverlay)
- 5c: Lightbox (PhotoLightbox, ExifPanel)
- 5d: Upload (UploadModal, DropZone, UploadProgressList) + LoginPage + useAuth

**Sync points:** shared `src/api/*.ts`, `src/data/copy.ts`, `src/lib/queryClient.ts`, `src/components/common/*` — these are written in Phase 4 so Phase 5 agents only consume.

### 14.4 Files that must NEVER be edited in parallel (merge conflict magnets)

Serialize these by routing all edits through one agent:

- `bootstrap/app.php` (Laravel 11+ collapsed config)
- `database/migrations/*` (filenames must be ordered)
- `app/Providers/Filament/AdminPanelProvider.php` (resource/widget registrations)
- `frontend/src/index.css` (`@theme` block)
- `frontend/src/App.tsx` (route definitions)
- `frontend/src/main.tsx` (provider tree)
- `.env.example` / `.env.production.example`
- `composer.json` / `package.json` (lockfile churn)
- `lefthook.yml` / `.claude/settings.json` / `.github/workflows/*`

Pattern: any of these touched in a parallel branch → rebase target gets the diff applied last by the integrator.

### 14.5 Coordination protocol for parallel agents

1. **Each track lives in its own git worktree** (Phase 12 sets this up).
2. **Each agent maintains a per-track `STATUS.md`** at the root of its worktree describing what it has done since branch and what's next. Read it before merging.
3. **Daily integration window** — at end of each "day" (or work session), each track opens a draft PR. The integrator (could be the human or a "rebase agent") merges in dependency order: A → B → C, resolving conflicts in the magnet files.
4. **CI gate per merge** — full test matrix (backend + frontend + E2E) must be green on the integrated branch before the next track lands.
5. **Stop-the-world events** — schema changes, dependency upgrades, env-var additions must pause parallel tracks; integrator broadcasts the change and tracks rebase.

---

## 15. Test-and-Commit Discipline

Every task in §13 (and every sub-task split out in §14) ships under the same discipline. This is non-negotiable: no commit lands without passing tests for the changed code.

### 15.1 What is "a task"?

A task is a single verifiable deliverable that can be committed in isolation. Examples:
- One migration
- One controller method (or one Form Request + Resource pair if they're tightly coupled)
- One Filament resource page
- One React component (or one custom hook)
- One job class
- One service class

**Anti-patterns** (refuse these as task definitions):
- "Implement the API" — too big; that's a phase
- "Wire it up" — no verification possible
- "Refactor stuff" — too vague
- "Fix the bug" — without a failing test, not a task

### 15.2 Test gates per task type

A task is **not done** until its tests are green. No "tests later." No `.skip`. No `--no-verify`.

| Task type | Required test before commit | Example |
|---|---|---|
| Migration | `php artisan migrate:fresh` succeeds; `migrate:rollback` succeeds | `<ts>_create_photos_table.php` |
| Eloquent model | Factory creates a row; relationship resolves; cast works | `PhotoFactory::new()->create()->album` returns an `Album` |
| Form Request | One test per validation rule (happy + each failure) | `StorePhotoRequest::test_rejects_oversize_file` |
| API endpoint | One feature test per documented status code in §6.6 | `POST /photos returns 202`, `… returns 415`, `… returns 422` |
| Service | Unit test with all dependencies stubbed | `ImageProcessor::generates_three_sizes` |
| Job | Feature test that runs the job synchronously and asserts side effects | `ProcessPhoto::sets_status_completed` |
| Filament resource | Smoke test with Livewire helpers | `livewire(ListPhotos::class)->assertCanSeeTableRecords(...)` |
| React hook | Vitest with `renderHook` | `useDebounce::emits_only_after_delay` |
| React component | RTL test for golden path + one error path | `UploadModal::shows_inline_error_for_oversize_file` |
| E2E flow | Playwright test for the full user journey | `upload → wait for processing → tile appears` |

### 15.3 Commit conventions

- **One task = one commit.** No "WIP" commits on shared branches.
- **Conventional Commits** format: `<type>(<scope>): <subject>`
  - Types: `feat`, `fix`, `chore`, `test`, `docs`, `refactor`, `perf`
  - Scopes: `backend`, `frontend`, `db`, `filament`, `api`, `pipeline`, `ci`, `spec`
  - Subject: imperative voice, ≤ 70 chars, no trailing period
- **Body** (optional but encouraged): why, not what. Reference SPEC.md sections.
- **Footer:** `Refs SPEC.md §6.2` or `Closes #123`

Example commit:
```
feat(api): add POST /photos with multipart upload

Implements SPEC.md §6.2 POST /photos contract — stores original
on photos_private disk, dispatches batched ProcessPhoto, returns
202 Accepted with Location header. Validation rules match
StorePhotoRequest in §10.1.

Refs SPEC.md §6.2 §11.1
```

### 15.4 Branch + PR workflow

- `main` is **protected** — direct push blocked (configured in §13 Phase 11).
- Feature branches: `feat/<phase>-<task>` (e.g. `feat/2-api-photos-store`).
- One PR per task (or per small task group if they share context). Never per phase.
- PR template requires:
  - **Summary** (1–3 bullets)
  - **Test plan** with checked boxes — every test gate from §15.2 ticked
  - **Screenshot** for any frontend change
  - **SPEC.md section reference**
- CI must pass: backend tests + frontend tests + lint + build. No exceptions.
- One reviewer required (multi-dev) OR one self-review pass with the `code-reviewer` agent (solo + agent setup).
- **Squash merge** — keeps `main` history one commit per merged PR.

### 15.5 Failure handling: revert, not skip

If a test introduced in Task N starts failing during Task N+M:
1. `git bisect` to find the breaking commit.
2. Either fix forward in a `fix:` commit, OR revert the breaking commit (`git revert <sha>`).
3. **Never** `--no-verify`, **never** delete the test, **never** mark `.skip`.

These are silent regressions. Treat them as production incidents.

### 15.6 Test coverage gates

- Backend: ≥ 80% line coverage (Pest `--coverage`, soft warn in CI).
- Frontend: ≥ 70% line coverage (Vitest v8, soft warn).
- These are floors, not targets. The intent is "every committed line was exercised by a test"; coverage % is a proxy. PR reviewers should look at *what* is covered, not just the number.
...
# Stateful domains only used if you migrate to Sanctum cookie mode (§9.4). Token mode ignores this.
SANCTUM_STATEFUL_DOMAINS=
SANCTUM_TOKEN_EXPIRATION=1440             # minutes — 24h (was 30 days; see §9.4)
...
DB_USERNAME=root
DB_PASSWORD=
DB_PERSISTENT_CONNECTIONS=false           # required for SQS workers (§11.5)

QUEUE_CONNECTION=database                 # prod: sqs
FILESYSTEM_DISK=photos
PHOTOS_DRIVER=local                       # prod: s3 (was PHOTOS_DISK; renamed for clarity)

# Image budget — informs queue worker --timeout (§11.5)
PHOTOS_MAX_DIMENSION=8000
PHOTOS_MAX_FILE_SIZE_KB=10240

# prod-only (uncomment + populate when PHOTOS_DRIVER=s3):
# SQS_KEY=...
# SQS_SECRET=...
# SQS_PREFIX=https://sqs.eu-west-1.amazonaws.com/...
...
# Logging
LOG_CHANNEL=stack
LOG_PROCESSING_CHANNEL=processing         # dedicated channel for ProcessPhoto exceptions (§10)
